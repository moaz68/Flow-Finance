...
fetch('../api/expenses/get_expenses.php') // أي ملف عندك بيستخدم Middleware
    .then(response => {
        if (response.redirected) { // لو الـ PHP عمل Redirect
    ...
    .then(data => {
        if (data && data.status === "error" && data.message === "Unauthorized access") {
            window.location.href = "pro.html"; // حوله لصفحة التسجيل
        }
    });
      ...
      <tr>
        ...
        <td>
          <div class="row-actions">
            <button class="btn-sm btn-edit" onclick="openEditModal(${id})" title="Edit"><i class="fa-solid fa-pen-to-square"></i></button>
            <button class="btn-sm btn-danger" onclick="deleteExpense(${id})" title="Delete"><i class="fa-solid fa-trash-can"></i></button>
          </div>
        </td>
      </tr>
    `;
...
function openEditModal(id) {
  ...
  document.getElementById('editCategory').value = expense.category || expense.Category || 'Other';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (editForm) {
    editForm.addEventListener('submit', async (e) => {
      ...
      try {
        const res = await fetch('../api/expenses/edit_expense.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id, title, amount, category })
        });
        const data = await res.json();
        if (data.status === 'success') {
          closeEditModal();
...
// --- EDIT EXPENSE LOGIC ---
function openEditModal(id) {
  const expense = window._fullExpensesList.find(e => e.id == id);
  if (!expense) return;
  ...
  if (editForm) {
    editForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (data.status === 'success') {
          closeEditModal();
          await fetchExpenses(); // This will refresh both tables
        } else {
          if (alertEl) alertEl.innerHTML = `<div class="alert error">${data.message || 'Failed to update'}</div>`;
        }
      } catch (err) {
        console.error('Edit expense error:', err);
        if (alertEl) alertEl.innerHTML = `<div class="alert error">Network error</div>`;
      } finally {
...
function updateMonthProgress() {
  ...
  if (messageEl) messageEl.innerText = `${t('label_stay_on_track')} ${daysRemaining} ${t('label_days')} ${t('msg_to_hit_goal')}`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (editNameForm) {
    editNameForm.addEventListener('submit', async (e) => {
      ...
      const btnText = document.getElementById('profileNameBtnText');
      if (profileAlert) profileAlert.innerHTML = '';
      if (nameError) { nameError.style.display = 'none'; nameError.textContent = ''; }

      const name = document.getElementById('profileName').value.trim();
      if (!name) {
        if (nameError) { nameError.textContent = 'Please enter a name.'; nameError.style.display = 'block'; }
        return;
      }

      if (saveBtn) saveBtn.disabled = true;
      if (spinner) spinner.style.display = 'inline-block';
      if (btnText) btnText.textContent = t('btn_saving');

      try {
        const resName = await fetch('../api/Settings/Modify_personal_data.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify({ name })
        });
        const ct = resName.headers.get('content-type') || '';
        let dataName;
        if (ct.includes('application/json')) {
          dataName = await resName.json();
        } else {
          dataName = { status: resName.ok ? 'success' : 'error', message: await resName.text() };
        }
        if (resName.ok && dataName.status !== 'error') {
          if (profileAlert) profileAlert.innerHTML = `<div class="alert success">✓ ${dataName.message || 'Name updated successfully.'}</div>`;
          setTimeout(() => { if (profileAlert) profileAlert.innerHTML = ''; }, 4000);
          if (typeof fetchUserProfile === 'function') fetchUserProfile();
        } else {
          if (profileAlert) profileAlert.innerHTML = `<div class="alert error">⚠ ${dataName.message || 'Failed to update name.'}</div>`;
        }
      } catch (err) {
  ...
  if (editEmailForm) {
    editEmailForm.addEventListener('submit', async (e) => {
      ...
      const saveBtn = document.getElementById('profileEmailSaveBtn');
      const spinner = document.getElementById('profileEmailSpinner');
      const btnText = document.getElementById('profileEmailBtnText');
      if (profileAlert) profileAlert.innerHTML = '';
      if (emailError) { emailError.style.display = 'none'; emailError.textContent = ''; }

      const email = document.getElementById('profileEmail').value.trim();
      if (!email) {
        if (emailError) { emailError.textContent = 'Please enter an email address.'; emailError.style.display = 'block'; }
        return;
      }
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      ...
      try {
        const resEmail = await fetch('../api/Settings/send_opt_change_email.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify({ email })
        });
        const ct = resEmail.headers.get('content-type') || '';
        let dataEmail;
        if (ct.includes('application/json')) {
          dataEmail = await resEmail.json();
        } else {
          dataEmail = { status: resEmail.ok ? 'success' : 'error', message: await resEmail.text() };
        }
        if (resEmail.ok && dataEmail.status !== 'error') {
          if (profileAlert) profileAlert.innerHTML = `<div class="alert success">✓ ${dataEmail.message || 'OTP Sent to new email.'}</div>`;
          document.getElementById('profileFormsContainer').style.display = 'none';
          document.getElementById('verifyEmailForm').style.display = 'block';
        } else {
          if (profileAlert) profileAlert.innerHTML = `<div class="alert error">⚠ ${dataEmail.message || 'Failed to send OTP.'}</div>`;
        }
      } catch (err) {
  ...
  if (verifyEmailForm) {
    verifyEmailForm.addEventListener('submit', async (e) => {
      ...
      if (submitBtn) submitBtn.disabled = true;
      if (spinner) spinner.style.display = 'inline-block';
      if (btnText) btnText.textContent = t('btn_verifying');

      try {
        const res = await fetch('../api/Settings/verify_change_email.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'same-origin',
          body: JSON.stringify({ email, opt: otp })
        });
        ...
        if (res.ok && data.status !== 'error') {
          if (alertEl) {
            alertEl.innerHTML = `<div style="color:var(--success); font-weight:600;">✓ ${data.message || 'Email verified successfully.'}</div>`;
            alertEl.style.display = 'block';
          }
          if (typeof fetchUserProfile === 'function') fetchUserProfile();
          setTimeout(() => {
            window.cancelVerifyEmail();
          }, 2000);
        } else {
          if (alertEl) {
            alertEl.innerHTML = `<div style="color:var(--danger); font-weight:600;">⚠ ${data.message || 'Failed to verify email.'}</div>`;
            alertEl.style.display = 'block';
          }
        }
      } catch (err) {
        ...
        if (alertEl) {
          alertEl.innerHTML = `<div style="color:var(--danger); font-weight:600;">⚠ Network error. Please try again.</div>`;
          alertEl.style.display = 'block';
        }
      } finally {
        if (submitBtn) submitBtn.disabled = false;
        if (spinner) spinner.style.display = 'none';
        if (btnText) btnText.textContent = 'Confirm Change';
      }
    });
  }

  // Global cancel function
  window.cancelVerifyEmail = function() {
    const vForm = document.getElementById('verifyEmailForm');
    const pForm = document.getElementById('profileFormsContainer');
    const profileAlert = document.getElementById('profileAlert');
    ...
    // Clear the previous "OTP Sent" message
    if (profileAlert) profileAlert.innerHTML = '';

    fetch('../api/Settings/verify_change_email.php', { 
        method: 'POST',
        headers: {
    ...
    .then(res => res.json())
    .then(data => {
        console.log("السيرفر استلم الإلغاء ونظف البيانات:", data);
        if (data.status === 'success' && profileAlert) {
            profileAlert.innerHTML = `<div class="alert success">✓ ${data.message || 'Canceled'}</div>`;
            setTimeout(() => { if (profileAlert) profileAlert.innerHTML = ''; }, 4000);
        }
    })
    .catch(err => {
        console.error("خطأ أثناء إرسال طلب الإلغاء:", err);
    });
};
...
async function loadExpensesSection() {
  ...
  if (statusEl) {
    ...
    statusEl.innerHTML = `<span style="display:inline-flex;align-items:center;gap:10px"><span class="spinner"></span> ${t('msg_loading_expenses')}</span>`;
  }
    ...
    html += `
      <tr>
        ...
        <td style="padding:13px 16px;background:var(--card);border-radius:0 10px 10px 0">
          <div class="row-actions">
            <button class="btn-sm btn-edit" onclick="openEditModal(${id})" title="Edit"><i class="fa-solid fa-pen-to-square"></i></button>
            <button class="btn-sm btn-danger" onclick="deleteExpense(${id})"><i class="fa-solid fa-trash-can"></i></button>
          </div>
        </td>
      </tr>`;
  });
    ...
    if (list.length === 0) {
      insightsHtml = `
        <div class="insight-card">
          ...
          <div><strong>${t('msg_getting_started')}</strong><br><span style="color:var(--muted)">${t('msg_add_expense_insight')}</span></div>
        </div>`;
    } else {
      ...
      if (topCatAmt > 0 && totalSpent > 0) {
        ...
        insightsHtml += `
          <div class="insight-card">
            ...
            <div><strong>${t('label_' + topCat.toLowerCase())} ${t('msg_biggest_expense')}</strong><br><span style="color:var(--muted)">${t('text_category_distribution').replace('{percent}', pct).replace('{timeframe}', timeframe === 'month' ? t('label_month') : timeframe === 'year' ? t('label_year') : t('label_day'))}</span></div>
          </div>`;
      }
      ...
      insightsHtml += `
        <div class="insight-card">
          ...
          <div><strong>${t('title_active_tracking')}</strong><br><span style="color:var(--muted)">${t('msg_recorded_expenses')} ${timeframe === 'month' ? t('label_month') : timeframe === 'year' ? t('label_year') : t('label_day')} ${t('msg_recorded_expenses_post')}.</span></div>
        </div>`;
...
async function fetchUserProfile() {
  ...
  const profileCurrencyBadge = document.getElementById('profileCurrencyBadge');

  const nameInput = document.getElementById('profileName');
  const emailInput = document.getElementById('profileEmail');

  // Show the header immediately with empty/loading state so it's visibly "added"
  if (profileHeader) {
  ...
  if (profileDisplayName) profileDisplayName.innerText = 'Loading...';
  if (profileDisplayEmail) profileDisplayEmail.innerText = 'Please wait';

  try {
    const res = await fetch('../api/get_settings_user.php');
    if (!res.ok) throw new Error('Failed to fetch user profile (Status: ' + res.status + ')');
    const data = await res.json();

    if (data && data.status === 'success') {
      let rawName = data.name || '';
      // Remove invisible formatting characters that might make the name appear empty
      ...
      if (profileRoleBadge) profileRoleBadge.innerText = role;
      if (profileCurrencyBadge) profileCurrencyBadge.innerText = currency;

      if (nameInput && !nameInput.value) nameInput.value = name;
      if (emailInput && !emailInput.value) emailInput.value = email;
    } else {
      console.warn('Failed to load profile data:', data);
      if (profileDisplayName) profileDisplayName.innerText = 'Error Loading Profile';
      if (profileDisplayEmail) profileDisplayEmail.innerText = data.message || 'Unknown error';
    }
  } catch (err) {
    console.error('Error fetching user profile:', err);
    if (profileDisplayName) profileDisplayName.innerText = 'Offline / Error';
    if (profileDisplayEmail) profileDisplayEmail.innerText = 'Could not reach server';
  } finally {
...
// ==========================================
window.cancelSettingsForgot = function () {
  const form1 = document.getElementById("settingsForgotStep1Form");
  const form2 = document.getElementById("settingsForgotStep2Form");
  const mainForm = document.getElementById("changePasswordFormEmbedded");
...
const settingsForgotBtn = document.getElementById("settingsForgotBtn");
if (settingsForgotBtn) {
  settingsForgotBtn.addEventListener("click", function () {
    const mainForm = document.getElementById("changePasswordFormEmbedded");
    const form1 = document.getElementById("settingsForgotStep1Form");
    const form2 = document.getElementById("settingsForgotStep2Form");
    const alert1 = document.getElementById("settingsForgotAlert1");
...
const settingsForgotStep1Form = document.getElementById("settingsForgotStep1Form");
if (settingsForgotStep1Form) {
  settingsForgotStep1Form.addEventListener("submit", function (e) {
    e.preventDefault();
    const btn = document.getElementById("settingsForgotSubmit1");
    const alertBox = document.getElementById("settingsForgotAlert1");

    // We get the email from the profile form which is pre-filled from the session
    const emailInput = document.getElementById("profileEmail") ? document.getElementById("profileEmail").value : "";
    if (!emailInput) {
      alertBox.innerText = "⚠️ البريد الإلكتروني غير متوفر. يرجى تحديث الصفحة أو حفظ الملف الشخصي أولاً.";
    ...
    btn.disabled = true;
    btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> ${t('btn_sending_otp')}`;
    ...
    fetch("../api/Settings/user/request_otp.php", {
      ...
      .then(data => {
        if (data.status === "error") {
          alertBox.innerText = data.message || "حدث خطأ غير معروف";
          alertBox.style.color = "#ef4444";
          alertBox.style.display = "block";
        } else {
          // Success
          document.getElementById("settingsForgotStep1Form").style.display = "none";
          document.getElementById("settingsForgotStep2Form").style.display = "block";
          const otpIn = document.getElementById("settingsForgotOtp");
          const newPassIn = document.getElementById("settingsForgotNewPass");
          const confirmPassIn = document.getElementById("settingsForgotConfirmPass");
      ...
      .catch(err => {
        alertBox.innerText = "⚠️ حدث خطأ أثناء الاتصال بالخادم.";
        alertBox.style.color = "#ef4444";
        alertBox.style.display = "block";
      })
      .finally(() => {
        btn.disabled = false;
        btn.innerText = t('btn_send_otp');
      });
  });
}

const settingsForgotStep2Form = document.getElementById("settingsForgotStep2Form");
if (settingsForgotStep2Form) {
  settingsForgotStep2Form.addEventListener("submit", function (e) {
    e.preventDefault();
    const btn = document.getElementById("settingsForgotSubmit2");
    const alertBox = document.getElementById("settingsForgotAlert2");

    const emailInput = document.getElementById("profileEmail") ? document.getElementById("profileEmail").value : "";
    const otpInput = document.getElementById("settingsForgotOtp").value;
    const newPassInput = document.getElementById("settingsForgotNewPass").value;
    const confirmPassInput = document.getElementById("settingsForgotConfirmPass").value;

   

    alertBox.style.display = "none";
    btn.disabled = true;
    btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> ${t('btn_resetting_pass')}`;

    fetch("../api/Settings/user/reset_password.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: emailInput, opt: otpInput, password: newPassInput, Confirm_Password: confirmPassInput })
    })
      .then(res => res.json())
      .then(data => {
        if (data.status === "error") {
          ...
          alertBox.style.display = "block";
          btn.disabled = false;
          btn.innerText = t('btn_confirm_change');
        } else {
          alertBox.innerText = data.message;
          alertBox.style.color = "#10b981";
          alertBox.style.display = "block";

          // Security cleanup: clear sensitive fields immediately after success
          const otpIn = document.getElementById("settingsForgotOtp");
          const newPassIn = document.getElementById("settingsForgotNewPass");
          const confirmPassIn = document.getElementById("settingsForgotConfirmPass");
          if (otpIn) otpIn.value = "";
