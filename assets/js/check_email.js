...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function startResendTimer() {
        ...
        resendBtn.innerHTML = `Resend Code <span id="countdown-timer">(${countdown}s)</span>`;
    ...
    resendBtn.addEventListener('click', async (e) => {
        ...
        // Immediately disable and show Sending...
        resendBtn.disabled = true;
        ...
        try {
            const response = await fetch('../api/resend_code.php', {
                ...
                body: JSON.stringify({}) // Session handles the email
            });

            const data = await response.json();

            if (data.status === 'success') {
                errorDisplay.textContent = data.message || 'A new security code has been dispatched.';
                errorDisplay.style.color = 'var(--primary-green)';
                errorDisplay.style.opacity = '1';
                
                setTimeout(() => {
                    errorDisplay.style.opacity = '0';
                    errorDisplay.style.color = 'var(--error-red)'; // Reset to default error color
                }, 3000);

                startResendTimer();
            } else {
                errorDisplay.textContent = data.message || 'Failed to resend the code. Please try again.';
