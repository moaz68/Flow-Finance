...
if(
!is_numeric($data["Food"]) || 
    !is_numeric($data["Bills"]) || 
    !is_numeric($data["Transport"]) || 
    !is_numeric($data["Shopping"]) || 
    !is_numeric($data["Entertainment"]) || 
    !is_numeric($data["Health"]) || 
    !is_numeric($data["Other"])
      ){
    echo json_encode([
            "status"=>"error",
